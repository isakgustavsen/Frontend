  ...
  <div class=" bg-slate-200  dark:bg-slate-900">
    <header class="bg-slate-800">
      <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
        <div class="flex lg:flex-1">
          <a href="/" class="-m-1.5 p-1.5">
            ...
            <!-- img class that uses logo in setup -->
      ...
      <Dialog as="div" class="lg:hidden" @close="mobileMenuOpen = false" :open="mobileMenuOpen">
        <div class="fixed inset-0 z-10"> test</div>
        <DialogPanel
    ...
    <footer class="bg-slate-800 inset-x-0 bottom-0">
      <div class="mx-auto max-w-7xl overflow-hidden px-6 py-20 sm:py-24 lg:px-8">
        <nav class="-mb-6 columns-2 sm:flex sm:justify-center sm:space-x-12" aria-label="Footer">
          <div v-for="item in footerNavigation.main" :key="item.name" class="pb-6">
            <a :href="item.href" class="text-sm leading-6 text-gray-200 hover:text-gray-300">{{ item.name }}</a>
          </div>
        </nav>
        ...
        <p class="mt-10 text-center text-xs leading-5 text-gray-300">&copy; 2023 DebutUKA i Tromsø All rights reserved.
        </p>
      </div>
    </footer>
  </div>
